...
;// CONCATENATED MODULE: ./src/assets/scripts/components/b2s/b2s-desk.js
/* provided dependency */ var b2s_desk_$ = __webpack_require__(19755);


var lastId,
  topMenu = b2s_desk_$(".section__nav"),
...
// Navigation swiper
slideNav = new core(".jsNav", {
  slidesPerView: 'auto',
  spaceBetween: 24,
  slideToClickedSlide: true
});
// scroll custom
b2s_desk_$(document).on("click", "[to-scroll]", function (e) {
  e.preventDefault();
  var link = b2s_desk_$(this).attr("to-scroll");
  if (b2s_desk_$(link).length > 0) {
    var posi = b2s_desk_$(link).offset().top - 50;
    b2s_desk_$("body,html").animate(
      {
        scrollTop: posi,
      },
      600
...
scrollItems = menuItems.map(function () {
  var item = b2s_desk_$(b2s_desk_$(this).attr("toscroll"));
  if (item.length) { return item; }
});

// Bind to scroll
b2s_desk_$(window).on('scroll', function () {
  // Get container scroll position
  var fromTop = b2s_desk_$(this).scrollTop() + topMenuHeight;

  // Get id of current scroll item
  var cur = scrollItems.map(function () {
    if (b2s_desk_$(this).offset().top < fromTop)
      return this;
  });
  ...
  if (lastId !== id) {
    ...
    menuItems
      ...
      .end().filter("[toscroll='#" + id + "']").parent().addClass("active");
  }

  navScrollToSection(menuItems)
    .then(slideNav.slideTo(slideIndex))
    .catch((error) => {
      console.error(error);
    });
});
...
function navScrollToSection(menuItems) {
  menuItems.on('click', function (e) {
    var href = b2s_desk_$(this).attr("toscroll"),
      offsetTop = href === "#" ? 0 : b2s_desk_$(href).offset().top - topMenuHeight + 1;
    b2s_desk_$('html, body').stop().animate({
      scrollTop: offsetTop
    }, 1000);
    e.preventDefault();
  });
...
// animate gift
b2s_desk_$(".modal-animation .gift").click(function () {
  b2s_desk_$(this).addClass("active");
  setTimeout(function () {
    b2s_desk_$(".modal-animation").click(function () {
      b2s_desk_$(this).removeClass("open").bind(".modal-animation");

    });
  }, 5000);
  setTimeout(function () {
    b2s_desk_$(".modal-animation").removeClass("open");
  }, 10000);
});
})();
