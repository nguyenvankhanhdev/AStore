                                            ...
                                            <tr>
                                                <td>
                                                    <div class="d-flex align-items-center">
                                                        <button class="btn btn-black btn-lg me-3 delete-wishlist"
                                                            data-id="{{ $item->id }}"
                                                            style="font-size: 1.5rem; padding: 0.5rem 0.8rem;">
                                                            <i class="fa fa-times"></i>
                                                        </button>
                                                        <img src="{{ asset($product->image) }}" alt="Product Image"
                                                            class="img-thumbnail" style="width: 80px; height: 80px;">
                                                        <div class="ms-3">
                                                            <p class="mb-1"><strong>{{ $product->name }} @if ($variant->storage->GB === '0GB')
                                                                    @else
                                                                        -{{ $variant->storage->GB }}
                                                                    @endif
                                                                </strong></p>
                                                            <p class="text-muted mb-0">Màu sắc:
                                                                {{ $variantColor->color->name }}</p>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td>
                                                    <p class="text-muted mb-1">
                                                        <del>{{ number_format($originalPrice, 0, ',', '.') }} đ</del></p>
                                                    <p class="text-danger fw-bold mb-1">
                                                        {{ number_format($discount, 0, ',', '.') }} đ</p>
                                                    <p class="text-success mb-0">Bạn tiết kiệm:
                                                        {{ number_format($offerPrice, 0, ',', '.') }} đ</p>

                                                </td>
                                                <td>
                                                    <button class="btn btn-link add-to-cart"
