...
//                                 //Cập nhật tin nhắn mới vào chat box
//                             const messageRow = $('tr[data-user-id="' + userId + '"]');
//                             messageRow.find('.message-preview').text(messageContent);
//                             messageRow.find('.time').html(`
//                                 ${new Date(messageTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true, hourCycle: 'h23' })}
//                             `);

//                             // Kiểm tra chat box
//                             const chatBoxDiv = $('#chat-box-' + userId);
//                             if (!chatBoxDiv.is(':visible')) {
//                                 // Chỉ thực hiện thay đổi trạng thái nếu có tin nhắn mới
//                                 if (messageRow.hasClass('read')) {
//                                     messageRow.removeClass('read').addClass('unread');

//                                     // Thêm chấm trạng thái và số lượng tin nhắn chưa đọc
//                                     messageRow.find('.status-dot').remove(); // Xóa nếu đã tồn tại
//                                     messageRow.find('.count-unread').remove(); // Xóa nếu đã tồn tại
//                                     messageRow.find('.time').append(`<span class="count-unread">(+ 1)</span>`); // Thêm số lượng tin nhắn chưa đọc
//                                     messageRow.find('.time').append('<span class="status-dot red"></span>'); // Thêm chấm đỏ
//                                 } else {
//                                     // Nếu đã là unread, cập nhật số lượng tin nhắn chưa đọc
//                                     const countUnreadElement = messageRow.find('.count-unread');
//                                     let currentCount = 0;

//                                     // Nếu tồn tại phần tử count-unread
//                                     if (countUnreadElement.length > 0) {
//                                         const match = countUnreadElement.text().match(/\d+/);
//                                         if (match) {
...
//                                     // Cập nhật số lượng tin nhắn chưa đọc
//                                     if (countUnreadElement.length === 0) {
//                                         messageRow.find('.time').append(`<span class="count-unread">(+ ${currentCount})</span>`);
//                                         messageRow.find('.time').append('<span class="status-dot red"></span>');
//                                     } else {
//                                         countUnreadElement.html(`(+ ${currentCount})`);
//                                     }
//                                 }
//                             }
//                             }
...
// test lặp qua từng chatbox
// $(document).ready(function() {
//     // Hàm để lấy tin nhắn mới cho từng người dùng
//     function fetchNewMessages() {
//         // Lặp qua từng chat box để cập nhật tin nhắn
//         $('.chat-box-custom').each(function() {
...
//                                             messageRow.find('.time').append(`<span class="count-unread">(+ 1)</span>`);
//                                             messageRow.find('.time').append('<span class="status-dot red"></span>');
//                                         } else {
//                                             const countUnreadElement = messageRow.find('.count-unread');
//                                             let currentCount = 0;
